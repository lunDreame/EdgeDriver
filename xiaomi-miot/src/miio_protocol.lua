...
local MiioProtocol = {}
MiioProtocol.__index = MiioProtocol
...
function MiioProtocol.new()
  local self = setmetatable({}, MiioProtocol)
  self.message_id = 0
...
function MiioProtocol:get_next_message_id()
...
function MiioProtocol:prepare_token(token_hex)
  local token = ""
...
function MiioProtocol:encrypt_message(data, token, token_key, token_iv, device_id, stamp)
...
function MiioProtocol:decrypt_message(data, token, token_key, token_iv)
  if #data < 32 then
...
function MiioProtocol:handshake(udp, ip)
  ...
  local success, err = udp:sendto(handshake_data, ip, MIIO_PORT)
  if not success then
...
function MiioProtocol:send_command(ip, token_hex, method, params, retry_count)
  params = params or {}
  ...
  for attempt = 1, retry_count do
    ...
    local encrypted_data = self:encrypt_message(command, token, token_key, token_iv, self.device_id, current_stamp)

    local success, err = udp:sendto(encrypted_data, ip, MIIO_PORT)
    if not success then
...
function MiioProtocol:get_device_info(ip, token)
...
function MiioProtocol:get_prop(ip, token, properties)
  local result = self:send_command(ip, token, "get_prop", properties)
  if result and result.result then
    return result.result
  end
  return nil
...
function MiioProtocol:send_raw(ip, token, method, params)
  local result = self:send_command(ip, token, method, params)
  if result and result.result then
    return result.result
  end
  return nil
